...
(`gas-childcare-visit-app/index.html`)と同じ機能・タブ構成のアプリが表示される。**画面の見た目・文言は
2026-09の UI/UX 改善提案(`doc/16_UIUX改善提案_2026-09-03.html`)に合わせて作り直しており、GAS版とは
意図的に違う**(詳細は下記「UI/UX改善提案への対応」)。「👪 お客様」タブは有効な顧客を全件取得し(顧客データは全て平文列。下記「データ保護の方針」
...
### UI/UX改善提案への対応(2026-09)

先方(株式会社キューテスト)から届いた改善提案 `doc/16_UIUX改善提案_2026-09-03.html` に沿って、
画面の言葉・大きさ・並びを作り直した。提案はGAS版 `index.html` を読んで書かれたものだが、
このReactアプリはGAS版の移植なので同じ指摘がそのまま当てはまる。
**「GAS版と同じ見た目にする」という従来の方針より、この提案が優先される**(下記「UIをGAS版に
合わせた範囲」のうち、見た目・文言に関する記述はこの節で上書きされている)。

想定利用者は50〜60代の訪問スタッフで、スマホはLINEと写真が主な用途。「取得」「反映」「同期」
「セッション」「OCR」といった語は通じず、10〜12pxの文字は読めない、という前提に立っている。

- **決めごとの置き場所**: 色・角丸は `packages/web/tailwind.config.js` の `theme.extend`(`app-primary`=
  進む/保存、`app-done`=完了/送った、`app-danger`=削除/事故、`app-subtle`=戻る/補助、`app-text`/`app-muted`=文字)。
  役割のない紫・エメラルド・琥珀・インディゴ・ティールは使わない。共通部品は `packages/web/src/ui/`
  (`Button`/`ButtonRow`/`StickyActionBar`、トーストと確認ダイアログの `useFeedback`、`LoadingBlock`、
  `EmptyState`、英文エラーを出さないための `toFriendlyMessage`)。新しいボタンや読み込み表示を
  自作せず、ここから使うこと。
- **フォント**: 英数字しか持たないOutfit(日本語は端末まかせで細く見える)をやめ、日本語がUD
  (ユニバーサルデザイン)書体になる **BIZ UDPGothic**(本文)と **Zen Maru Gothic 700**(見出し・ボタン)を
  `@fontsource` で同梱する。日本語の実体は1書体あたり約1.3MBあるため、PWAの先読み(precache)からは
  外し、一度使ったものを実行時キャッシュに残す(`vite.config.ts` の `globIgnores`/`runtimeCaching`)。
  外部ドメインへリクエストを出さない方針は従来どおり。
- **文字サイズ**: 本文の基準を16px・行間1.6にし、`text-[8px]`〜`text-[11px]` と `text-xs`(12px)は使わない
  (取りこぼしても14px未満にならないよう `src/index.css` で下限を入れてある)。切りかえは
  「ふつう16 / 大きい18 / とても大きい20」の3段階で、**既定は「大きい」**(一番小さいものを既定に
  しない)。localStorageのキー `app_text_size` と値(`small`/`medium`/`large`)はGAS版から変えていないので、
  既に設定しているスタッフの選択はそのまま引き継がれる。ヘッダーの「Aa」ボタンで順送りできる。
- **押せるものの大きさ**: ボタンは高さ48px以上(補助でも44px)、間隔12px以上。入力欄の文字は16px未満に
  しない(iOSが自動でズームし、戻らなくなるため)。アイコンだけのボタンを作らない(「⚙️」ではなく「⚙️ 設定」)。
  `hover:` は使わず `active:` にする(スマホにマウスオーバーは無い)。
- **言葉**: 「顧客」→「お客様」、「勤怠/勤怠シート/過去の予定」→「出勤簿」、「取得/反映/読込/再取得」→
  「見る/取り込む/最新にする」に統一。英文の技術メッセージ(`Error: ...`)は画面に出さず、
  `toFriendlyMessage` で「うまくいきませんでした。電波を確認して、もう一度押してください」に
  言いかえて原因は console に出す。読み込み中は必ず文字を添え、失敗のお知らせは自動で消さない
  (成功は4秒)。

### UIをGAS版に合わせた範囲(機能・導線の対応関係)

以下はGAS版のどの機能をどう移植したかの対応表であって、**見た目・文言は上記「UI/UX改善提案への対応」で
作り直されている**(タブ名・ボタン名・配色は現物と異なる箇所がある)。
...
- **スタイルの配信方法はGAS版と違う**: TailwindはCDNではなくビルド時に生成し(`packages/web/tailwind.config.js`/
  `postcss.config.js`/`src/index.css`、Tailwind v3)、フォントも`@fontsource`で同梱している
  (書体そのものは上記のとおりUD書体へ入れ替えた)。画面表示にあたって外部ドメインへ出ていく
  リクエストは無い(顧客情報を扱う画面から第三者のCDNへアクセスが飛ばない)。
...
- **領収書(実費報告)の一覧と取り消し**: 勤怠タブの「🧾 領収書」から、自分が登録した領収書を
  ...
  (`POST /api/receipts/:id/cancel`)。会計の記録なので金額や紐付け先を書き換える経路は作らず、
  取り消しても行は消さずに一覧へグレーで残し、いつ・誰が・なぜ取り消したかを表示する。
  取り消せるのは**`min(領収書の日付 + receipt_cancellable_days, 締め日)`**まで(既定2日。
  暦日で数える。訪問保育は土日祝日も訪問があるため営業日では数えない)。期限は画面
  (ボタンを出さない)とサーバーの両方で見る。管理者だけは期限後も取り消せる(経理が締め処理で
  戻すため)。**締め日はテナントごとの管理者設定**(`app_settings` の `receipt_closing_day` /
  `receipt_cancellable_days`。設定画面から変更する)。締め日は月末か1〜28日から選ぶ
  (29〜31は2月に存在しないため選べない)。
  期限は登録時に確定して `receipts.cancellable_until` に保存し、以後は設定を変えても動かさない。
  **この列を持たない古い行だけは例外**で、保存値が無いぶん現在の設定から計算する。つまり
  「新規登録分は登録時の設定で固定、旧行だけ現在の設定に追随する」の2通りになる。
  **ミラー送信は遅らせない**(登録と同時に送信対象になる)。スプレッドシートは移行期の一時的な
  写しでしかなく、遅らせると「まだ移行が済んでいない人が見ている間だけシートが空」という、
  ミラーの目的と正反対の状態になるため。取り消し済みを送らない守りは、送信の開始を
  「`cancelled_at IS NULL` の行に `mirror_claimed_at` を立てるUPDATE」として表し、取り消しの
  UPDATEと同じ行ロックで直列化することで担う(`claimForMirror`)。取り消しが先なら送信は
  始まらず、送信の宣言が先なら取り消しの応答でそれを返して画面に出す。`mirror_claimed_at` が
  表すのは送信を**開始した**ことまでで、HTTP送信の成否は分からないため、画面も「送信済み、
  または送信中」と出して手でシートを確認してもらう。
