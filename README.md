...
- **領収書(実費報告)の一覧と取り消し**: 勤怠タブの「🧾 領収書」から、自分が登録した領収書を
  ...
  取り消せるのは**領収書の日付 + `receipt_cancellable_days`(既定2日。暦日で数える。訪問保育は
  土日祝日も訪問があるため営業日では数えない)**、かつ**送信日の前日**までで、期限は画面
  (ボタンを出さない)とサーバーの両方で見る。管理者だけは期限後も取り消せる(経理が締め処理で
  戻すため)。期限は登録時に確定して `receipts.cancellable_until` に保存し、以後は設定を
  変えても動かさない(送信予定だけが取り残されて「送ったあとに取り消せる」状態になるのを防ぐ)。
  **この列を持たない古い行だけは例外**で、保存値が無いぶん現在の設定から計算する。つまり
  「新規登録分は登録時の設定で固定、旧行だけ現在の設定に追随する」の2通りになる。
  **スプレッドシートへのミラー送信は、この取り消し期限が切れるまで始めない**(送ってしまうと
  取り消してもあちら側に残るため)。
  **締め日はテナントごとの管理者設定**(`app_settings` の `receipt_closing_day` /
  `receipt_mirror_lead_days` / `receipt_cancellable_days`。設定画面から変更する)。
  締め日は月末か1〜28日から選ぶ(29〜31は2月に存在しないため選べない)。送信日は
  「締め日の `receipt_mirror_lead_days` 日前」で、既定の1なら締め日の前日から送れる状態になる
  = 締める時点で当期分がすべてシートに出ている想定(0なら締め日当日が送信日)。決まるのは
  送信開始の時刻で、実際に出るのはワーカーが拾ったときです(失敗すれば再試行のぶんずれます)。月末締めならこれで**前月分が翌月に送られること
  自体が無い**。締め間際の領収書は取り消せる期間がゼロになり、登録と同時に送信対象になる。
  期限内の取り消しは必ず送信より前に起きる。**管理者の期限後取り消しだけはその外**にあるため、
  送信の開始を「`cancelled_at IS NULL` の行に `mirror_claimed_at` を立てるUPDATE」として表し、
  取り消しのUPDATEと同じ行ロックで直列化している(`claimForMirror`)。取り消しが先なら送信は
  始まらず、送信の宣言が先なら取り消しの応答でそれを返して画面に出す。`mirror_claimed_at` が表すのは
  送信を**開始した**ことまでで、HTTP送信の成否は分からないため、画面も「送信済み、または送信中」と
  出して手でシートを確認してもらう。
  そのぶん「まだ送っていない」期間が必ずできるので、一覧に未送信の件数・送信予定時刻・
  送信失敗の件数を出して、締めのときに取り残しへ気付けるようにしている。
