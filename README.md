...
- **世帯構成員のアレルギー**: 自由記述とは別に `allergy_status`(未確認/なし/あり)+ `allergy_note` の
  ...
  clearContents していて、手入力が再取込で消えていた)。同姓同名が2人いる世帯では取り違えを避けて
...
- **日報・事故報告の書きかけの復元**: 保存前の入力を端末に控え、訪問先一覧に
  ...
  入力が丸ごと消えるため。GAS版は問答無用でダイアログを開いていたが、別の訪問をしようとして
  開いた場面では紛らわしいので、案内を出して選んでもらう形にした。保存が済んだ時点で控えは消す。
- **顧客に紐付かない経費の領収書**: 駐車場代などを顧客を選ばずに登録できる(GAS版
  `openStandaloneReceiptModal` の移植)。DBとAPIは元から `customer_id` を nullable にして
  対応していたが、画面からの入口が無かった。このモードでは請求区分を「会社立替」に固定する
