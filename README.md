...
- **領収書(実費報告)の一覧と取り消し**: 勤怠タブの「🧾 領収書」から、自分が登録した領収書を
  ...
  取り消せるのは領収書の日付+2日(暦日。訪問保育は土日祝日も訪問があるため営業日では数えない)、
  かつ**送信締切**までで、期限は画面(ボタンを出さない)とサーバーの両方で見る。管理者だけは
  期限後も取り消せる(経理が締め処理で戻すため)。
  ...
  **締め日はテナントごとの管理者設定**(`app_settings` の `receipt_closing_day` /
  ...
  締め日は月末か1〜28日から選ぶ(29〜31は2月に存在しないため選べない)。送信締切は
  「締め日の `receipt_mirror_lead_days` 日前」で、既定の1なら締め日の前日いっぱいで送り終える
  = 締める時点で当期分がすべてシートに出ている。月末締めならこれで**前月分が翌月に送られること
