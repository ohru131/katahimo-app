...
phases = [
    ...
    ("スキーマのみ", VIOLET, VIOLET_L,
     ...
     "日報AIの3軸は表・制約・組み立てロジックはあるが、生成への接続・リポジトリ・API・画面はまだ無い"),
    ("未着手", RED, RED_L,
...
# 10. ポート一覧
# ══════════════════════════════════════════════════════════════
s = sl_("窓口(ポート)の一覧 — 30本", "core が「外にこれをやってほしい」と宣言しているものの全体",
        source="packages/core/src/ports/*.ts")
groups = [
    ("データの読み書き(16本)", ACCENT, ACCENT_L,
...
rows = [
    ...
    ["AIプロンプトの文面を管理者が編集", "実装済み",
     "文面はDBで版管理(上書きしない)。管理画面から編集・履歴・既定に戻すまで動く"],
    ["予約・請求・カルテ・日報AIの3軸", "スキーマのみ", "表・制約・組み立てロジック(3軸)はある。生成への接続・リポジトリ・API・画面はこれから"],
...
items = [
    ...
    ("③", "先行整備した6ドメインの進め方", VIOLET,
     "予約・請求・カルテ・訪問最適化・移動手当は表と制約のみ。日報AIの3軸は組み立てロジックまである。"
