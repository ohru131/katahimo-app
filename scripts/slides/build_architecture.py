...
card(s, ML, 4.85, 6.0, 1.35, "この配置の狙い", accent=GREEN, items=[
    {"t": [("PostgreSQLが唯一の正データ", {"bold": True}),
           (". スプレッドシートは写しで、止めるときはミラーの部品を外す", {})]},
...
for ttl, col, fl, items in groups:
    ...
    bullets(s, cx + 0.2, 1.85, w - 0.4, 3.7, [{"t": it, "s": 10.5} for it in items],
            size=10.5, line=1.2, gap=3.5, marker_color=col)
    cx += w + 0.14
...
card(s, ML + 6.33, 2.85, 6.0, 2.05, "書き込み側の保護", accent=ACCENT, items=[
    ...
    {"t": "パスワードの差し替えは、行をロックして差し替えとセッション破棄を1つの"
          "トランザクションで行う"},
    {"t": "ログインの成否・パスワード変更・管理者操作を、実行者と対象付きでログに残す"},
], body_size=10.5)
...
card(s, ML + 6.33, 2.7, 6.0, 2.0, "本番ビルドに混ざらないことの担保", accent=ACCENT, items=[
    ...
    {"t": "デモデータは前回開いた日の翌日から当日まで自動で追加する(間隔が空いても履歴が続く)"},
], body_size=10.5)
...
card(s, ML, 1.28, 6.0, 3.3, "やっていること", accent=GREEN, items=[
    ...
    {"t": [("制約が実際に効くこと", {"bold": True}),
           ("  CHECK・一意制約・複合外部キーが不正なINSERTを拒否することを、本番と同じ"
            "マイグレーションを当てたDB上で確認(拒否理由が意図した制約名であることまで見る)", {})]},
