...
phases = [
    ...
    ("スキーマのみ", VIOLET, VIOLET_L,
     "予約 / 請求・決済 / 顧客カルテ / 訪問割当の最適化 / 移動手当 / 日報AIの3軸(年齢帯・教育関心度・ストレス度)"
     "(表と制約はあるが、実装・API・画面は無い)"),
    ("未着手", RED, RED_L,
...
rows = [
    ...
    ["予約・請求・カルテ・日報AIの3軸", "スキーマのみ", "表・制約・ドキュメントはある。リポジトリ実装・API・画面はこれから"],
...
items = [
    ...
    ("③", "先行整備した6ドメインの進め方", VIOLET,
     "予約・請求・カルテ・訪問最適化・移動手当・日報AIの3軸は、表と制約だけ先に用意してある。"
