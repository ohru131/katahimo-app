...
card(s, ML + 8.3, 1.25, 4.03, 2.5, "規模", accent=GREEN, items=[
    ...
    {"t": [("TypeScript 229ファイル / 約27,500行", {"bold": True}), ("(テストを除く)", {})]},
    {"t": [("テスト 56ファイル / 728件", {"bold": True}), ("。CIで毎回実行", {})]},
    {"t": [("データベース 34テーブル", {"bold": True}), (" / マイグレーション1本", {})]},
], body_size=10.5)
...
# 18. テスト戦略
# ══════════════════════════════════════════════════════════════
s = sl_("何をテストし、何をしていないか", "テスト56ファイル / 728件。分布は意図的に偏らせている",
...
card(s, ML, 1.28, 6.0, 4.6, "この構成で守れていると考えているもの", accent=GREEN, items=[
    ...
    {"t": "毎pushで静的検査・型検査・728件のテスト・本番ビルド・混入検査まで自動実行"},
