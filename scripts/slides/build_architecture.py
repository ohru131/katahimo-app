...
title_slide(
    ...
    "2026-09 時点 / データベース設計は doc/12_データベース構造レビュー資料.pptx を参照",
)
...
s = sl_("何を、なぜ作り直しているか", "背景と現在地",
        source="doc/07_技術構成提案書.md / doc/10_開発メモ / README「フェーズ状況」")
...
card(s, ML + 4.15, 1.25, 4.0, 2.5, "目指す先", accent=ACCENT, items=[
    ...
    {"t": [("一斉切り替えを避ける", {"bold": True}), (":Phase 0〜8の段階移行", {})]},
], body_size=10.5)
card(s, ML + 8.3, 1.25, 4.03, 2.5, "規模(2026-09)", accent=GREEN, items=[
    {"t": [("9パッケージ", {"bold": True}), ("(pnpm workspace)", {})]},
    {"t": [("TypeScript 204ファイル / 約20,600行", {"bold": True}), ("(テストを除く)", {})]},
    {"t": [("テスト 43ファイル / 343件", {"bold": True}), ("。CIで毎回実行", {})]},
    {"t": [("データベース 13テーブル", {"bold": True}), (" / マイグレーション8本", {})]},
], body_size=10.5)
...
phases = [
    ...
    ("未着手", RED, RED_L,
     "本番デプロイ(Cloud Run) / 予約・請求・カルテ / Cloud KMS / 再暗号化バッチ / 並行運用の照合"),
]
cy = 4.3
for nm, col, fl, body in phases:
    rect(s, ML, cy, CW, 0.72, fill=fl, border=None)
    rect(s, ML, cy, 0.075, 0.72, fill=col, border=None, shape=MSO_SHAPE.RECTANGLE)
    text(s, ML + 0.22, cy + 0.22, 2.5, 0.3, nm, size=12, color=col, bold=True)
    text(s, ML + 2.9, cy + 0.14, CW - 3.2, 0.55, body, size=11, color=INK, line=1.3)
    cy += 0.8
text(s, ML, 6.72, CW, 0.3,
     "本資料で「未検証」「未着手」と書いたものは、提案書では実現済みのように読めてしまう部分です。先に区別をお伝えします。",
     size=11, color=MUTED)
...
# 10. ポート一覧
# ══════════════════════════════════════════════════════════════
s = sl_("窓口(ポート)の一覧 — 26本", "core が「外にこれをやってほしい」と宣言しているものの全体",
        source="packages/core/src/ports/*.ts")
groups = [
    ("データの読み書き(12本)", ACCENT, ACCENT_L,
...
card(s, ML, 2.7, 6.0, 2.0, "本番と同じものが動いている", accent=GREEN, items=[
    {"t": "同じマイグレーション(13テーブル)・同じ行レベルセキュリティ・同じ業務ロジック・同じAPIルート"},
...
# 18. テスト戦略
# ══════════════════════════════════════════════════════════════
s = sl_("何をテストし、何をしていないか", "テスト43ファイル / 343件。分布は意図的に偏らせている",
...
card(s, ML, 1.28, 6.0, 3.3, "やっていること", accent=GREEN, items=[
    ...
    {"t": [("鍵の世代", {"bold": True}),
           ("  交換後も旧世代の暗号文を復号できること、解約後はどの世代も読めないこと", {})]},
...
card(s, ML + 6.33, 1.28, 6.0, 3.3, "やっていないこと", accent=RED, items=[
    ...
    {"t": [("実PostgreSQLに対するテストが無い", {"bold": True}),
           ("  リポジトリ層はブラウザ内DBで検証しており、本番ドライバ固有の挙動差は未確認", {})]},
    {"t": [("勤怠の実データ照合が未実施", {"bold": True}),
           ("  給与に直結する領域。実際の出勤簿との突き合わせはPhase 7の予定", {})]},
...
s = sl_("ドキュメントの記述と、実装の実態", "誤解を招きやすい点を先にお伝えします",
        source="README「フェーズ状況」/ doc/07 / packages/db/src/schema")
rows = [
    ["PostgreSQL + RLS のマルチテナント", "実装済み", "13テーブル、複合外部キー、FORCE。静的検査と実DB検証をCIで実行"],
    ...
    ["予約・請求・カルテ・機能フラグ", "未着手", "提案書の差別化要因だが、スキーマにテーブルすら無い"],
    ["勤怠計算はGAS版と一致", "合成データのみ", "実際の出勤簿での照合はPhase 7"],
]
colmap = {"実装済み": GREEN, "稼働中": GREEN, "方針変更": AMBER, "未検証": AMBER,
          "未着手": RED, "合成データのみ": AMBER}
cc = {}
for i, r in enumerate(rows):
    cc[(i, 1)] = colmap[r[1]]
table(s, ML, 1.28, CW, ["ドキュメントが語っていること", "実装", "実際の状態"], rows,
      col_w=[4.4, 1.6, 6.5], size=10.5, hsize=10.5, row_h=0.48, header_h=0.33, first_bold=True,
...
items = [
    ...
    ("③", "予約・請求・カルテをどう積むか", RED,
     "提案書の差別化要因が、まだスキーマにも無い。二重予約の防止や請求の整合性は、"
     "あとから足すと既存データの移行が伴う。",
     "いま土台に入れるか、要件が固まるまで後回しにするか"),
...
text(s, ML, 6.78, CW, 0.3,
     "私の推奨:メール送信だけ別経路に逃がしたうえで、予約のスキーマを先に入れること。ただし意見ですので、ご判断をいただきたいです。",
