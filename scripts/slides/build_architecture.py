...
# 2. この資料の目的と、先に押さえる4語
# ══════════════════════════════════════════════════════════════
s = sl_("この資料の目的と、基本の4語", "はじめに",
        source="用語は本資料での説明を優先し、正確な定義はコードに委ねます")
card(s, ML, 1.25, 6.0, 1.45, "この資料で説明すること", accent=ACCENT, items=[
    {"t": "どこに何が書いてあるか、なぜその置き方にしたか"},
...
text(s, ML, 2.9, CW, 0.3, "本資料で使う4語", size=13, color=INK, bold=True)
terms = [
    ("モノレポ", "1つのリポジトリに複数の部品を入れる作り",
     "9つの部品(パッケージ)が同じリポジトリに入っている。共通の型やロジックを、"
     "コピーせずに共有できる", ACCENT, ACCENT_L),
    ("ポート / アダプタ", "窓口と、外とつなぐ部品",
     "業務ロジックはメールを送る窓口だけを知り、SendGridかGmailかは知らない。"
     "差し替えとテストがしやすくなる", VIOLET, VIOLET_L),
    ("ヘキサゴナル", "業務ロジックを中心に置き、外側を差し替え可能にする設計",
     "データベースも外部APIも画面も、中心から見れば外側。"
     "依存の矢印は中心へ向かう一方通行になる", GREEN, GREEN_L),
    ("Outbox", "送信予定をデータベースに書いておき、後で送る方式",
     "スプレッドシートへの書き戻しに使う。送信が失敗しても、業務データの保存は"
     "完了できる", ORANGE, ORANGE_L),
]
cx = ML
for nm, short, body, col, fl in terms:
    ...
    cx += w + 0.14
note(s, ML, 6.1, CW, 0.8, "ポート / アダプタが中心",
     "ポート / アダプタは本資料の中心にある考え方です。この作りにした理由は、"
     "同じ業務ロジックを本番(Node.js + PostgreSQL)とブラウザ内デモ(ブラウザ内DB)で"
     "そのまま動かせるようにするためです(P16)。",
...
card(s, ML, 1.25, 4.0, 2.5, "現行:Google Apps Script 版", accent=MUTED, items=[
    {"t": "データはスプレッドシート / Drive / カレンダー"},
    {"t": [("1法人・1Googleアカウント", {"bold": True}), ("に強く依存", {})]},
    {"t": "認証は共有ソルトのSHA-256。トークンはシートに直書き"},
    {"t": "顧客CSVの取込は全置換(欠損時に丸ごと消える)"},
    {"t": "本番稼働中。機能追加は凍結し不具合修正のみ"},
], body_size=10.5)
card(s, ML + 4.15, 1.25, 4.0, 2.5, "目指す先", accent=ACCENT, items=[
    {"t": [("マルチテナントSaaS化", {"bold": True}), ("。複数法人を1基盤に収容", {})]},
    {"t": [("PostgreSQLを唯一の正データ", {"bold": True}),
           ("にし、スプレッドシートは互換用の写しとして残す", {})]},
    {"t": "将来は訪問看護事業へ展開(カルテ・保存年限)"},
    {"t": [("一斉切り替えを避け", {"bold": True}), ("、GAS版と併存しながら段階的に移す", {})]},
], body_size=10.5)
card(s, ML + 8.3, 1.25, 4.03, 2.5, "規模", accent=GREEN, items=[
    ...
    {"t": [("データベース 44テーブル", {"bold": True}), (" / マイグレーション6本", {})]},
], body_size=10.5)

text(s, ML, 3.95, CW, 0.3, "実装の現状", size=13, color=INK, bold=True)
phases = [
    ("実装済み", GREEN, GREEN_L,
     "認証 / 顧客 / 日報・事故報告 / 領収書 / 勤怠(月次集計・カレンダー反映) / スタッフ管理 / RESERVA取込 / "
     "割引クーポン / AIプロンプト文面の編集(版管理)・日報AIの3軸(年齢帯×教育関心度×ストレス度)まで接続 / 公開デモ"),
    ("コードはあるが未検証", AMBER, AMBER_L,
     "スプレッドシートへの書き戻し、メール送信(GASブリッジが未デプロイのため動作未確認)"),
    ("スキーマのみ", VIOLET, VIOLET_L,
     "予約 / 請求・決済 / 顧客カルテ / 訪問割当の最適化 / 移動手当は表と制約のみ"),
    ("未着手", RED, RED_L,
...
text(s, ML, 6.78, CW, 0.3,
     "未検証・未着手と書いたものは、提案書では実現済みのように読める部分です。ここで区別しておきます。",
...
box(s, 6.7, 2.6, 2.8, 1.0,
    [("GAS ブリッジ", {"size": 12, "bold": True}), ("\n", {}),
     ...
     ("まだデプロイしていない", {"size": 9.5, "color": RED})],
...
card(s, ML, 4.85, 6.0, 1.5, "この配置の狙い", accent=GREEN, items=[
    {"t": [("PostgreSQLが唯一の正データ", {"bold": True}),
           ("。スプレッドシートは写しで、ミラーを外せば止められる", {})]},
    {"t": "Google系の連携は既存のGASブリッジ1本に集約(課金・認証情報の新設を避けるため)"},
], body_size=10.5)
note(s, ML + 6.33, 4.85, 6.0, 1.5, "この配置の弱点(第4章の判断事項①)",
     "メール・地図・予定・スプレッドシートの4系統がGASブリッジ1本に集中しています。"
     "実行時間制限とクォータがあり、SLAはなく、単一障害点です。まだデプロイしていません。",
     accent=RED, fill=RED_L, size=10.5)
rect(s, ML, 6.45, CW, 0.48, fill=CARD, border=None)
text(s, ML + 0.2, 6.53, CW - 0.4, 0.3,
     "赤い注記はまだ存在しないもの。破線はリクエストの完了を待たずに後から動く処理。"
...
rows = [
    ...
    ["データベース", "PostgreSQL", "行レベルセキュリティ・排他制約など、必要な制約をDB側で強制できる", ""],
    ["ORM", "Drizzle ORM 0.45", "テーブル定義をTypeScriptで書き、SQLとの型の食い違いを防ぐ", ""],
    ["APIフレームワーク", "Hono 4", "軽量。Node.jsでもブラウザ内でも同じアプリを動かせる(デモで活用)", ""],
    ["画面", "React 18 + Vite + Tailwind CSS + PWA", "スマホでの現場利用が主。ホーム画面に追加して使える", ""],
...
table(s, ML, 1.28, CW, ["領域", "採用したもの", "選んだ理由", "備考"], rows,
      col_w=[1.8, 3.3, 5.3, 2.1], size=10, hsize=10.5, row_h=0.43, header_h=0.33,
      first_bold=True,
      cell_colors={(5, 3): AMBER, (7, 3): AMBER, (9, 3): RED})
note(s, ML, 6.48, CW, 0.44, "",
     "備考欄には、提案書の方針から変更した箇所と未配備のものだけを記載しています。変更の理由は各章で説明します。",
...
# ══════════════════════════════════════════════════════════════
sec_("第 2 章", "コードの構造",
     "9つの部品と、その間の依存の向き。業務ロジックをどこに置いたか")

# ══════════════════════════════════════════════════════════════
# 8. パッケージ構成
# ══════════════════════════════
s = sl_("9つの部品と依存の向き", "矢印は依存の方向。逆方向の依存はない",
...
box(s, 3.6, 3.3, 4.6, 0.95,
    [("core  業務ロジック", {"size": 13, "bold": True}), ("\n", {}),
     ("usecases / domain / ports。外部SDKを知らない", {"size": 10})],
...
text(s, ML + 0.2, 6.53, CW - 0.4, 0.3,
     "デモ用コードは本番ビルドに混ざらない。入口を空の実装に差し替え、CIが成果物を検査する。",
     size=10.5, color=ORANGE, bold=True)

# ══════════════════════════════
# 9. ヘキサゴナル
# ══════════════════════════════════════════════════════════════
s = sl_("業務ロジックと外側の実装", "ポートとアダプタ(ヘキサゴナルアーキテクチャ)",
...
box(s, 4.65, 3.55, 4.0, 1.35,
    [("ports  外に対する窓口の定義", {"size": 10.5, "bold": True}), ("\n", {}),
     ("保存する・送る・暗号化するといった\n"
      "形(インターフェース)を33本定義する。\n中身は書かない", {"size": 10})],
...
note(s, ML, 5.75, 6.0, 1.15, "この構造の利点",
     "外部サービスが無い環境では、エラーにせず何もしない実装で動く(地図・予定・AI・ミラー)。"
     "同じ業務ロジックが、本番でもブラウザ内デモでもそのまま動く。",
     accent=GREEN, fill=GREEN_L, size=11)
note(s, ML + 6.33, 5.75, 6.0, 1.15, "ポート定義の例外",
     "パスワードのハッシュ化の窓口は、ports ではなく usecases 側に定義しています。"
...
# 10. ポート一覧
# ══════════════════════════════════════════════════════════════
s = sl_("窓口(ポート)の一覧 — 33本", "core が外部に求める処理を宣言したもの",
        source="packages/core/src/ports/*.ts")
groups = [
    ("データの読み書き(19本)", ACCENT, ACCENT_L,
     ["Tenant テナント", "Staff スタッフ", "Session セッション", "PasswordResetCode 再設定コード",
      ...
      "AppSettings 管理者設定", "PromptTemplate AIプロンプト文面", "TenantKey 暗号鍵",
      "ReportAiConfig 日報AIの3軸設定", "CustomerReportProfile 家庭ごとの★",
      "ReportAiGeneration AI生成の記録"]),
...
for ttl, col, fl, items in groups:
    ...
    # 19本の一覧を箱(高さ4.35)に収めるため、字を少し詰める
    bullets(s, cx + 0.2, 1.82, w - 0.4, 3.75, [{"t": it, "s": 9.5} for it in items],
            size=9.5, line=1.05, gap=1.5, marker_color=col)
    cx += w + 0.14
card(s, ML, 5.8, 6.0, 1.1, "窓口だけを定義する効果", accent=GREEN, items=[
    {"t": "テストでは偽の実装を渡すだけで済む。データベースも外部APIも起動しないため、"
          "842件のテストが約35秒で終わる"},
], body_size=10.5)
note(s, ML + 6.33, 5.8, 6.0, 1.1, "ポートの集約(Container)",
     "全ポートの実装を1つにまとめ、それを usecase に渡します。"
     "Node.js用とブラウザ用の2種類を、同じ型で用意しています。",
     accent=ACCENT, fill=ACCENT_L, size=10.5)

# ══════════════════════════════════════════════════════════════
# 11. リクエストを1本追う
# ══════════════════════════════════════════════════════════════
s = sl_("リクエストを1本追う", "日報を保存するときに何が起きるか",
...
steps = [
    ...
    ("② 入口の検査", "api(入口)", "Cookie・権限・Origin・入力を検査"),
    ("③ 業務の手順", "core(業務)", "usecase が日報を保存する手順を実行"),
    ("④ 1つのトランザクション", "db / 外部", "日報の行と送信予定(outbox)を同時に確定"),
...
text(s, ML + 0.2, 6.52, CW - 0.4, 0.3,
     "④で日報の行と送信予定を同じトランザクションで確定させる。片方だけ通ると、"
     "検知できない取り残しが残る。",
     size=11, color=INK)
...
# ══════════════════════════════════════════════════════════════
sec_("第 3 章", "外部との連携",
     "現行のGAS版との共存、認証、AI、公開デモ")

# ══════════════════════════════════════════════════════════════
# 13. GAS版との共存
# ══════════════════════════════════════════════════════════════
s = sl_("現行のGAS版とどう共存するか", "正データはPostgreSQL、スプレッドシートは写し",
...
card(s, ML, 2.35, 6.0, 2.08, "この方針にした理由(doc/proposal/gas-bridge.md)", accent=GREEN, items=[
    {"t": "既存のGASブリッジを優先。①新規の課金・認証情報を避ける "
          "②実証済みのロジック(イベント分類・タグ解析・氏名照合)を再実装しない"},
    {"t": "スプレッドシートを止めるときは、ミラーのアダプタを外す(業務ロジックは無変更)"},
    {"t": "多重起動しても安全(SKIP LOCKED)。処理中のまま5分経った行は再取得される"},
], body_size=10.5)
card(s, ML + 6.33, 2.35, 6.0, 2.08, "送信失敗時の再試行", accent=ACCENT, items=[
    {"t": "送信に失敗しても捨てない。5秒から倍々(上限1時間)で再試行し、8回で失敗扱いにする"},
    {"t": "失敗扱いになった件数はエラーログに出るため、運用側で把握できる"},
    {"t": "同じ内容を二重に送らないよう、送信予定には一意キー(idempotency_key)を持たせている"},
], body_size=10.5)

text(s, ML, 4.6, CW, 0.3, "この依存のしかたに残る問題(判断事項①)", size=12.5, color=RED, bold=True)
probs = [
    ("4系統が1本に集中", "メール・地図・予定・スプレッドシートがGAS Web App 1本を経由。"
                   "実行時間制限・クォータあり、SLAなし"),
    ("まだデプロイしていない", "スプレッドシートへの書き戻しとメール送信は、"
                          "本番で動かしたことがない。メール送信の処理はGAS側にまだ無い"),
    ("secret がURLに載る", "GASはヘッダーを読めないため、認証用の文字列をURLクエリに付ける。"
                     "ログに残り、交換手順も未整備"),
...
note(s, ML, 6.35, CW, 0.6, "選択肢",
     "(a) この依存のまま早期にデプロイして運用設計を詰める  (b) メール送信だけ別経路(SES / SendGrid等)に分ける  "
...
text(s, ML, 1.2, CW, 0.3,
     "変更した理由は、GAS版の既存パスワードを引き継ぎ、スタッフに再設定を求めないため。"
     "初回ログイン時に、古い方式のハッシュを新方式へ入れ替える。",
     size=11.5, color=INK)
y = 1.62
...
for ttl, body, col, fl in flow:
    ...
    cx += w + 0.2
card(s, ML, 2.85, 6.0, 2.05, "総当たりと列挙への対策", accent=GREEN, items=[
    {"t": "連続10回の失敗で15分ロック、成功でリセット。恒久ロックにはしない"
          "(狙って失敗させれば人を締め出せるため)"},
    {"t": "ロック中かどうかで応答を変えない(アカウントの有無が分かるため)"},
    {"t": "再設定は、存在しない宛先でも同じ応答を返す(ユーザー列挙を塞ぐ)"},
    {"t": "6桁コードは検証子だけを保存。30分・誤入力5回で無効"},
], body_size=10.5)
card(s, ML + 6.33, 2.85, 6.0, 2.05, "書き込み側の保護", accent=ACCENT, items=[
    {"t": "初期パスワードのまま他のAPIを叩くと403(入口の1か所で判定)"},
    {"t": "書き込み系は Origin を照合してクロスサイトからの要求を弾く"},
    {"t": "パスワードの差し替えは、行をロックして差し替えとセッション破棄を1つの"
          "トランザクションで行う"},
    {"t": "ログイン成否・パスワード変更・管理者操作を、実行者と対象付きで記録"},
], body_size=10.5)
text(s, ML, 5.0, CW, 0.3, "残っている弱点", size=12.5, color=RED, bold=True)
weak = [("制限がアカウント単位のみ", "IPや端末単位のレート制限は前段(WAF / Cloud Armor)が必要"),
...
text(s, ML + 0.2, 6.6, CW - 0.4, 0.3,
     "テストは、失敗すべき経路で確実に失敗することを中心に書いています。",
     size=10.5, color=INK)

# ══════════════════════════════════════════════════════════════
# 15. AIと通知
# ══════════════════════════════════════════════════════════════
s = sl_("AI(Gemini)と通知の扱い", "生成物だけでなく、元の入力も残している",
...
card(s, ML, 2.65, 6.0, 1.85, "設計上の判断", accent=GREEN, items=[
    {"t": [("生成前の口語メモも列として保存する", {"bold": True}),
           (". 生成物だけを残すと、元の記述を後から確かめられない", {})]},
    {"t": "AIが使えない環境では、何もしない実装になる(画面は生成なしで動く)"},
...
card(s, ML + 6.33, 2.65, 6.0, 1.85, "APIキーと通知先の持ち方", accent=ACCENT, items=[
    ...
    {"t": [("この3項目だけはアプリ側で暗号化している", {"bold": True}),
           ("(業務データは平文。詳細は doc/slides/db-review.pptx P15)", {})]},
...
text(s, ML + 0.2, 6.53, CW - 0.4, 0.3,
     "AIは下書きを作る道具として位置づけ、最終的な内容の責任はスタッフが持つ形にしています。",
     size=10.5, color=INK)

# ══════════════════════════════════════════════════════════════
# 16. 公開デモ
# ══════════════════════════════════════════════════════════════
s = sl_("公開デモ — 同じコードをブラウザ内で動かす", "core が外部SDKを知らないことの実証でもある",
...
card(s, ML, 2.7, 6.0, 2.0, "本番と共通の部分", accent=GREEN, items=[
    {"t": "同じマイグレーション(44テーブル)・同じ行レベルセキュリティ・同じ業務ロジック・同じAPIルート"},
    {"t": "差し替えているのは、Node.js専用の実装と、鍵を置けない外部API(AI・地図・メール)だけ"},
    {"t": "Service Worker ではなく fetch の差し替えにしたのは、GitHub Pages の"
          "サブパス配信で適用範囲の問題を避けるため"},
], body_size=10.5)
card(s, ML + 6.33, 2.7, 6.0, 2.0, "本番ビルドに混ざらないことの担保", accent=ACCENT, items=[
    {"t": "本番ビルドではデモの入口を空の実装に差し替える(ビルド設定)"},
    {"t": [("設定ではなく、出来上がった成果物をCIが検査する", {"bold": True}),
           (". ブラウザ内DBのファイルが含まれていないか、禁止文字列が無いかを確認", {})]},
    {"t": "デモの暗号鍵は公開された固定値。訪問者が入力したAPIキー等は永続化しない"},
    {"t": "デモデータは前回開いた翌日から当日まで自動で追加(履歴が途切れない)"},
], body_size=10.5)
text(s, ML, 4.85, CW, 0.3, "デモ固有の制約(本番には影響しない)", size=12.5, color=AMBER,
     bold=True)
bullets(s, ML, 5.2, CW, 1.1, [
    {"t": "PGlite は1.0未満。書き出しの完了を待つために内部の仕組みを直接呼ぶため、"
          "バージョンを固定している"},
...
# 17. §4
# ══════════════════════════════════════════════════════════════
sec_("第 4 章", "品質と現状", "何をテストしていて、何をしていないか。判断いただきたいこと")
...
card(s, ML, 1.28, 6.0, 3.3, "やっていること", accent=GREEN, items=[
    {"t": [("ドメインの純関数の回帰", {"bold": True}),
           ("  勤怠計算はGAS版をNodeで実行した結果と、合成データ19ケース+月次集計で完全一致。"
            "CSV解析は実サンプル398行で一致", {})]},
    {"t": [("usecase の単体テスト", {"bold": True}),
           ("  偽の実装を渡し、失敗すべきときに失敗することを中心に検証(列挙防止・試行上限・"
            "割り込み・部分更新なし)", {})]},
    {"t": [("トランザクション境界", {"bold": True}),
           ("  送信予定への書き込みだけを失敗させ、日報の行が残らないことを固定", {})]},
    {"t": [("行レベルセキュリティ", {"bold": True}),
           ("  全テーブルの静的検査と、権限を落としたロールでの実挙動", {})]},
    {"t": [("制約が実際に効くこと", {"bold": True}),
           ("  CHECK・一意制約・複合外部キーが不正なINSERTを拒否することを、本番と同じ"
            "マイグレーションを当てたDBで確認(制約名まで照合)", {})]},
...
note(s, ML, 4.72, 6.0, 1.18, "テストの書き方の方針",
     "新しく足したテストは、直した箇所を元に戻すと落ちることを1つずつ確認してから残しています。"
     "通るだけのテストは、後から読む人に誤解を与えます。",
     accent=GREEN, fill=GREEN_L, size=11)
...
# 19. 主張と実装の対応
# ══════════════════════════════════════════════════════════════
s = sl_("ドキュメントの記述と、実装の実態", "誤解を招きやすい点",
        source="README「実装状況」/ doc/proposal/tech-stack.md / packages/db/src/schema")
rows = [
    ["PostgreSQL + RLS のマルチテナント", "実装済み", "44テーブル、複合外部キー、FORCE。静的検査と実DB検証をCIで実行"],
    ...
    ["Sheetsミラーの実送信・メール送信", "未検証",
     "GASブリッジが未デプロイのため動作未確認。既定でミラーは無効"],
    ["Cloud KMS による鍵管理", "未着手", "鍵を包む鍵は環境変数のまま。再暗号化バッチも無い"],
    ["AIプロンプトの文面編集・日報AIの3軸(年齢帯×教育関心度×ストレス度)", "実装済み",
     "文面はDBで版管理。3軸は表・管理画面・生成API・保存・記録(保持期間つき)まで接続済み"],
    ["予約・請求・カルテ・訪問割当の最適化・移動手当", "スキーマのみ", "表と制約のみ。リポジトリ・API・画面はこれから"],
...
table(s, ML, 1.28, CW, ["ドキュメントが語っていること", "実装", "実際の状態"], rows,
      col_w=[4.4, 1.6, 6.5], size=10.5, hsize=10.5, row_h=0.39, header_h=0.31, first_bold=True,
      cell_colors=cc, aligns=[PP_ALIGN.LEFT, PP_ALIGN.CENTER, PP_ALIGN.LEFT])
note(s, ML, 6.0, CW, 0.9, "提案書と実装のずれ",
     "提案書だけを読むと、すでに揃っているように見えます。赤い行は未着手です。"
     "ご判断いただきたいのは、この土台の上に赤い行を積んでよいかどうかです。"
     "土台に構造的な問題があれば、積む前に直します。",
     accent=ACCENT, fill=ACCENT_L)

# ══════════════════════════════════════════════════════════════
# 20. 判断事項
# ══════════════════════════════════════════════════════════════
s = sl_("ご判断いただきたいこと", "優先度順。いずれも直せない問題ではなく、実施時期の判断",
        source="データベース設計の論点は doc/slides/db-review.pptx 第3〜4章")
items = [
    ("①", "GASブリッジ1本への依存", RED,
     "メール・地図・予定・スプレッドシートの4系統を1つのGAS Web Appに集約。SLAは無く、"
     "認証用の文字列がURLに載る。まだデプロイしていない。",
     "(a) この依存のまま早期にデプロイして運用設計を詰める / (b) メールだけ別経路に分ける / "
    ...
    ("③", "先行整備した5ドメインの進め方", VIOLET,
     "予約・請求・カルテ・訪問最適化・移動手当は表と制約のみ(日報AIの3軸は実装済みで対象外)。"
     "予約の二重取りはDBで止められず、アプリ側の責任。",
...
for no, ttl, col, body, choice in items:
    ...
    yy += 1.1
text(s, ML, 6.78, CW, 0.3,
     "推奨は、メール送信を別経路に分けたうえで予約から実装に進めることです。ご判断をお願いします。",
     size=11, color=INK, bold=True)

# ══════════════════════════════════════════════════════════════
# 21. まとめ
# ══════════════════════════════════════════════════════════════
s = sl_("まとめ", "左は現状できていること、右は判断いただきたいこと")
card(s, ML, 1.28, 6.0, 4.6, "この構成でできていること", accent=GREEN, items=[
    {"t": "業務ロジックが外部SDKを知らないため、同じコードがNode.jsとブラウザの"
          "両方で動く(公開デモで実証)"},
    {"t": "依存の矢印は中心へ向かう一方通行。逆方向の依存はない(package.json で確認)"},
    {"t": "対で成立すべき書き込みは、1つのトランザクションで確定する。"
          "崩れたらテストが落ちる"},
    {"t": "テナント分離が二重(アプリ側とデータベース側)。効いていることを静的検査と実DBの両方で検証"},
    {"t": "認証の失敗経路(列挙・総当たり・割り込み・部分更新・クロスサイト)への対策を入れた"},
    {"t": "44テーブルで同じスキーマ規約。新しい表を足す手順が決まっている"},
