...
# 10. ポート一覧
# ══════════════════════════════════════════════════════════════
s = sl_("窓口(ポート)の一覧 — 33本", "core が「外にこれをやってほしい」と宣言しているものの全体",
...
card(s, ML + 6.33, 2.7, 6.0, 2.0, "本番ビルドに混ざらないことの担保", accent=ACCENT, items=[
    ...
    {"t": "デモデータは前回開いた日の翌日から「今日」まで自動で追い足す(何日か空けても履歴が続く)"},
], body_size=10.5)
...
card(s, ML, 1.28, 6.0, 4.6, "この構成で守れていると考えているもの", accent=GREEN, items=[
    ...
    {"t": "44テーブルすべてで同じスキーマ規約。新しい表を足す手順が決まっている"},
