...
items = [
    ...
    ("自動で守られること", GREEN, GREEN_L,
     "システム側で守られる決めごと。入力ミスがあっても、この内容に反する登録は"
     "できないようになっています。"),
...
s = sl_("データベースとExcelの言葉の対応", "第1章 Excelに置き換える")
table(s, ML, BODY_TOP, CW,
      ...
      [
          ...
          ["重複の削除・チェック", "重複禁止", "一意制約", "同じものが2件登録されるのを防ぐ"],
...
card(s, ML + 6.36, BODY_TOP, 6.16, 2.55, "記録として残ること", accent=GREEN,
...
note_(s, ML, 5.0, CW, "つながりが防ぐこと",
     "予約シートに、存在しない顧客の管理番号を書くことはできません。"
     "また、予約が残っている顧客を誤って削除することもできません。"
     "Excelでいえば「VLOOKUPが #N/A になる状態」を作れないようにしてあります。\n"
     "これにより、顧客名簿には無いのに日報だけ残っているような、"
...
# 第2章 全体像
# ══════════════════════════════════════════════════════════════
sec_("第2章", "全体像 — 44枚のシートの地図", "業務の流れのどこで、どのシートに記録が残るか")

s = sl_("業務の流れと、記録が残る場所", "第2章 全体像")
steps = [
...
note_(s, ML, 3.12, CW, "",
     "上の6つが業務の流れです。それぞれの段階で、その下に書いたシートに記録が残ります。"
...
card(s, ML, 4.08, 4.06, 2.72, "流れの外にあるもの", accent=MUTED, items=[
    ...
    {'t': [("特性・相性", {'bold': True}), ("(第12章) — 割当を自動で組むための材料", {})]},
...
rules = [
    ("金額は「円の整数」で持つ", GREEN,
     "1円未満は持ちません。消費税の端数処理は決めた場所で1回だけ行うので、"
     "Excelの表示桁による誤差は起きません。"),
    ...
    ("登録日時・更新日時が自動で入る", ACCENT,
     "入力する人が意識しなくても、いつ作られ、いつ直されたかが残ります。"),
    ("法人ごとに分かれている", ACCENT,
...
note_(s, ML, 6.08, CW, "",
     "上の12件は、いずれも業務の決めごとであって、システムの都合ではありません。"
...
card(s, ML, BODY_TOP, 4.06, 2.4, "この資料の内容", accent=ACCENT, items=[
