...
table(s, ML, BODY_TOP, CW,
      ...
      [
          ...
          ["1枚のシート", "シート", "テーブル", "今回は34枚。「顧客」「予約」「日報」など内容ごとに分かれる"],
...
# 第2章 全体像
# ══════════════════════════════════════════════════════════════
sec_("第2章", "全体像 — 34枚のシートの地図", "業務の流れのどこで、どのシートに記録が生まれるか")
...
for d in DOMAINS:
    if d['no'] == '3':
        sec_("第3章-第14章", "業務ごとに、記録する項目を確認する",
...
# 第15章 全体で守っている決めごと
# ══════════════════════════════════════════════════════════════
sec_("第15章", "全体で守っている決めごと", "どの章にも共通してかかっているルールです")

s = sl_("34枚すべてに共通する7つの決めごと", "第15章 全体の決めごと")
rules = [
    ("金額は「円の整数」で持つ", GREEN,
...
s = sl_("「消さない」「後から動かない」が効く場面", "第15章 全体の決めごと",
...
table(s, ML, BODY_TOP, CW,
      ...
      col_w=[0.9, 6.6, 4.5], size=11, hsize=11, row_h=0.40, header_h=0.34, first_bold=True)
note_(s, ML, 6.08, CW, "",
     "上の11件は、いずれも「業務の決めごと」であって、システムの都合ではありません。"
     "決まり次第、そのとおりに作ります。現時点で決まっているものがあれば教えてください。",
     accent=AMBER, fill=AMBER_L)

s = sl_("作りかけのもの(入れ物はあるが画面が無い)", "第16章 まだ決まっていないこと")
card(s, ML, BODY_TOP, 6.16, 2.3, "画面まで動くもの", accent=GREEN, items=[
    {'t': "顧客・世帯構成員の登録と一覧"},
    {'t': "保育日報・事故報告の登録"},
...
card(s, ML + 6.36, BODY_TOP, 6.16, 2.3, "記録する場所だけ決めてあるもの", accent=AMBER, items=[
...
note_(s, ML, 3.75, CW, "「作りかけ」だからこそ、いま見ていただきたい",
...
# 第17章 レビューの進め方
# ══════════════════════════════════════════════════════════════
sec_("第17章", "ご確認の進め方", "お手数をおかけしますが、よろしくお願いいたします")

s = sl_("ご確認の進め方", "第17章 ご確認の進め方")
steps = [
