...
title_slide(
    ...
    "katahimo-app\nデータベース構造レビュー資料",
    "訪問保育(ベビーシッター法人)向け業務SaaS — PostgreSQL / 44テーブル",
...
card(s, ML, 1.25, 3.95, 2.35, "お願いしたいこと", accent=ACCENT, items=[
    ...
    {"t": "危ないところを指摘いただくことが目的"},
], body_size=11.5)
card(s, ML + 4.15, 1.25, 3.95, 2.35, "設計者の前提", accent=AMBER, items=[
    {"t": [("データベース設計は今回が初めて", {"bold": True}), ("。用語の使い方自体が間違っている可能性がある", {})]},
    {"t": "実装は動いており、テストとCIも回っている"},
    {"t": "知らないために選ばなかった選択肢があるはず、という前提で見ていただきたい"},
], body_size=11.5)
card(s, ML + 8.3, 1.25, 4.03, 2.35, "この資料の作り", accent=GREEN, items=[
    ...
    {"t": [("赤い枠", {"color": RED, "bold": True}), ("は自覚している弱点を示す", {})]},
], body_size=11.5)

text(s, ML, 3.9, CW, 0.3, "資料の構成(全30ページ)", size=13, color=INK, bold=True)
chs = [
    ("第1章", "まず用語から", "表・主キー・外部キー・\nトランザクション・RLS", ACCENT, ACCENT_L, "P4–5"),
    ("第2章", "現状の構成", "44テーブルの全体像と、\nテナント分離・データ保護", GREEN, GREEN_L, "P6–18"),
    ("第3章", "設計上の問題点", "customers 39列の肥大化ほか\n6件を自己申告", RED, RED_L, "P19–25"),
    ("第4章", "相談事項", "先行して用意したスキーマと、\n判断いただきたい論点", VIOLET, VIOLET_L, "P26–30"),
]
cx = ML
...
note(s, ML, 6.28, CW, 0.66, "詳細の参照先",
     "用語の使い方が違うという指摘も歓迎です。設計の詳細は doc/db/overview.md、"
...
# ══════════════════════════════════════════════════════════════
s = sl_("用語① テーブル・行・列・キー", "データベースは関係づけられた表の集まり",
...
bullets(s, ML, 1.25, 5.9, 4.6, [
    ...
    {"t": [("外部キー(FK)", {"bold": True, "color": GREEN, "size": 13}),
           ("  … 別の表の行を指す紐付け。日報がどの顧客の記録かを持つのに使う。"
            "存在しない顧客IDを書くとデータベースが拒否する", {"size": 12})]},
    {"t": [("表を分ける理由", {"bold": True, "size": 13}),
           ("  … 顧客の住所を1か所に置けば、住所が変わったとき直すのは1行だけで済む。"
            "日報ごとに住所を書き写していたら、直し漏れが出る(これを", {"size": 12}),
...
note(s, DX, 5.42, 5.85, 1.15, "この2枚の読み方",
     "以降の図の矢印は、ほぼすべて外部キーによる紐付けです。矢印の向きは"
     "参照する側 → 参照される側。tenant_id 列は次章で説明します。",
     accent=ACCENT, fill=ACCENT_L, size=11)
...
rows = [
    ["UNIQUE(一意制約)", "この列の組み合わせを重複禁止にするルール",
     "同じメールで2人登録できないようにする"],
    ["CHECK(検査制約)", "列に入ってよい値をデータベース側で縛るルール",
     "区分値・1〜5の評価・0以上の金額をDBが拒否する"],
    ["INDEX(索引)", "本の巻末索引。全ページ読まずに目的の行へ飛べる",
     "姓が佐藤の顧客を全件走査せず引く"],
    ["トランザクション", "複数の書き込みをまとめて確定/取消する単位",
     "日報の保存と通知予約を同時に確定する"],
    ["RLS(行レベルセキュリティ)", "データベース側で見える行を絞り込む機能",
     "他社のデータは見えなくする"],
...
note(s, DX, 4.95, 4.3, 1.15, "重要な理由",
     "日報は保存できたのに通知だけ飛ばない、という状態を防ぐため。"
     "片方だけ成立すると、どの記録が取り残されたのか追えなくなる。",
...
# ══════════════════════════════════════════════════════════════
sec_("第 2 章", "現状の構成",
     "44テーブルの全体像と、テナント分離・データ保護・トランザクションの考え方")
...
card(s, ML, 4.15, 6.0, 1.35, "後から書き換えない記録が多い", accent=GREEN, items=[
    {"t": "日報・事故報告・領収書・勤怠は、その日の事実を積み上げる性質。削除は行わず、"
          "顧客の退会も deactivated_at を立てるだけ(ソフトデリート)"},
], body_size=11.5)
card(s, ML + 6.2, 4.15, 6.13, 1.35, "①〜④の外側に、先に形だけ固めた5領域がある", accent=ORANGE, items=[
...
note(s, ML, 5.72, CW, 1.1, "設計の出発点",
     ...
     "これを複数法人(テナント)が同居できる形に作り替えるのが今回の目的です。したがって他社のデータが混ざらないことが"
     "設計上の最優先事項です。",
...
# 9. テーブル一覧① 稼働中の17枚
# ══════════════════════════════════════════════════════════════
s = sl_("テーブル一覧① — 稼働中の17枚", "アプリが実際に読み書きしている表と、鍵になる制約",
...
rows = [
    ...
    ["prompt_templates", "AI生成のプロンプト文面。版を積む", "UNIQUE(tenant_id, key, version)。無い版は既定文面へ", "○"],
...
rows3 = [
    ...
    ["report_age_band_keywords", "表", "年齢帯と相性の良い語の対応(多対多)。PK=(tenant_id, age_band_id, keyword_id)"],
    ["report_phrases", "表", "温かみ表現(encourage)と全日報で避ける表現(avoid)。適用するストレス度の範囲を持つ"],
    ["report_education_levels", "基準", "教育関心度★(1〜5)ごとの教育語の使い方。上限語数(0〜3)と専門語の可否"],
    ["report_stress_levels", "基準", "ストレス度(1〜5)ごとの制限。★の引き下げ幅・教育語の可否・連絡要否の3列"],
    ["customer_report_profiles", "設定", "家庭ごとの★。PK=(tenant_id, customer_id)。customers の列にはしない"],
    ["report_ai_generations", "記録", "AI生成1回。モデル・使った版・★/ストレス度・入力・生の出力。updated_atを持たない"],
    ["report_ai_generation_keywords", "記録", "その生成で提示した候補(candidate)とAIが実際に使った語(used)"],
]
kindcol = {"表": ACCENT, "基準": GREEN, "設定": PINK, "記録": VIOLET}
cc3 = {(i, 1): kindcol[r[1]] for i, r in enumerate(rows3)}
table(s, ML, 1.25, CW, ["テーブル", "区分", "役割と、鍵になる制約"], rows3,
      col_w=[3.3, 1.1, 7.85], size=10, hsize=10.5, row_h=0.33, header_h=0.31,
      first_bold=True, cell_colors=cc3,
      aligns=[PP_ALIGN.LEFT, PP_ALIGN.CENTER, PP_ALIGN.LEFT])

text(s, ML, 4.62, CW, 0.28, "保護者向けの文面を組み替える3つの軸", size=12, color=INK, bold=True)
axes = [
    ("① 子の年齢帯", ACCENT, "生年月日と訪問日から月齢を都度計算し、report_age_bands の帯に当てる"),
    ("② 家庭の教育関心度★(1〜5)", GREEN, "高いほど教育語を使う。家庭ごとに管理者・担当者が付ける"),
    ("③ 保護者のストレス度(1〜5)", RED, "低いほど負担が大きい。訪問ごとにスタッフが評価(daily_reports.stress_level)"),
]
AW = (CW - 0.32) / 3
cx = ML
for ttl, col, body in axes:
    rect(s, cx, 4.92, AW, 1.06, fill=WHITE, border=col, border_w=1.2)
    text(s, cx + 0.16, 5.01, AW - 0.32, 0.26, ttl, size=11, color=col, bold=True)
    text(s, cx + 0.16, 5.30, AW - 0.32, 0.60, body, size=10, color=INK, line=1.3)
    cx += AW + 0.16
note(s, ML, 6.06, CW, 0.86, "ストレス度は★より優先する(制限を文面ではなく列で持つ)",
     "負担の大きい家庭では教育語を止めてねぎらいに切り替えます。「★を何段下げるか」「教育語を使ってよいか」"
     "「管理者連絡を要するか」は report_stress_levels の3列に持たせ、コードが機械的に適用します"
     "(文面の一節として書くと、文面の編集で制限が外れるため)。",
...
opts = [
    ("方式A 共有スキーマ + tenant_id 列", ACCENT, ACCENT_L, True,
     ["1つのデータベース・1組の表に全社のデータを入れ、各行に tenant_id 列でどの法人かを持たせる",
...
note(s, ML + 6.33, 4.45, 6.0, 2.1, "方式Aの弱点への対策(次の2ページ)",
     "行が混ざりうるという方式Aの弱点に対して、二重の仕組みを入れています。"
     "① この法人の行しか返さない条件をデータベース側に設定する(RLS)  "
     "② 紐付けを2列セットにして、同じ法人の行かをデータベース側で検査する(複合外部キー)",
     accent=AMBER, fill=AMBER_L, size=11)

# ══════════════════════════════════════════════════════════════
# 11. RLS
# ══════════════════════════════════════════════════════════════
s = sl_("守り① データベース側で行を絞り込む", "RLS(Row Level Security / 行レベルセキュリティ)",
...
box(s, ML + 5.25, y, 2.9, 0.95,
    [("SET LOCAL", {"font": MONO, "size": 10.5, "bold": True}), ("\n", {}),
     ...
     ("この接続の現在の法人を宣言", {"size": 9.5, "color": MUTED})],
...
text(s, ML, y2 + 1.88, 6.0, 0.5,
     "設定を忘れた接続からは、どの行も見えない。書き込みも同じ条件で弾かれる。",
     size=10.5, color=MUTED, line=1.3)

card(s, ML + 6.33, 2.95, 6.0, 1.45, "効いていることをCIで毎回検査している", accent=GREEN, items=[
    {"t": "静的検査:全44テーブル分のSQLに ENABLE と FORCE、ポリシーの条件が揃っているかを機械的に検査。"
          "新しい表を足して書き忘れると落ちる"},
...
text(s, ML + 0.25, 3.62, 5.55, 1.2,
     ...
     "T社の日報がU社の顧客を指す行ができ、RLSで見えないため気づけない。",
...
text(s, ML + 6.58, 3.62, 5.55, 1.2,
     "T社かつそのIDという組み合わせが参照先に無ければ、データベースが書き込みを拒否する。"
     "アプリにバグがあっても、法人を跨いだ参照は作れない。",
     size=11, color=INK, line=1.32)

text(s, ML, 4.72, CW, 0.3,
     [("この形を適用した参照(全63本)", {"size": 12, "bold": True, "color": INK}),
      ("   下は稼働中の17枚にあるもの", {"size": 10.5, "color": MUTED})])
...
text(s, ML, 3.95, CW, 0.3, "業務データを守る4層", size=12.5, color=INK, bold=True)
layers = [
...
note(s, ML, 5.75, CW, 1.1,
     ...
     "本番(Cloud Run + Cloud SQL)は未配備で、いま動いているのはローカル開発(Docker、保存時暗号化なし)と"
...
text(s, ML, 1.22, CW, 0.3,
     "鍵を1本だけ使うと、その鍵が漏れたときに全社分が読めてしまう。そこで法人ごとに別の鍵を作り、"
     "その鍵をさらに上位の鍵で包んで保管する。",
     size=12, color=INK)
y = 1.68
box(s, ML, y, 3.2, 1.0,
...
box(s, ML + 8.4, y, 3.93, 1.0,
    [("暗号文(app_settings 3列)", {"size": 11.5, "bold": True, "color": GREEN}), ("\n", {}),
     ...
     ("2列ペア。どの世代の鍵で暗号化したかを必ず記録", {"size": 10})],
...
bullets(s, ML, 4.25, 6.0, 1.3, [
    {"t": "1法人1行だと、鍵を交換した時点で既存の暗号文が読めなくなる(交換が実質不可能)"},
...
card(s, ML + 6.33, 2.95, 6.0, 1.6, "法人ごとに独立生成する理由", accent=ACCENT, items=[
    {"t": "マスター鍵と法人IDから鍵を計算する方式にすると、マスター鍵が漏れれば"
          "全法人の鍵を再現できる。鍵を1本共有しているのと変わらない"},
...
note(s, ML, 5.75, CW, 1.1, "ご相談したいこと(論点⑥)",
     "世代を並存させたので鍵の交換自体はできますが、既存データを新しい世代へ移す再暗号化バッチを作っていません。"
     "読めなくはならない代わりに、古い世代の鍵を捨てられない状態です。"
...
# 15. トランザクションとOutbox
# ══════════════════════════════════════════════════════════════
s = sl_("トランザクションと Outbox", "日報1本を保存するとき、何と何を同時に確定させているか",
...
text(s, ML + 2.35, 1.28, 4.0, 0.24, "1つのトランザクション(まとめて確定)", size=10, color=GREEN,
     bold=True)
...
card(s, ML, 4.62, 6.0, 1.2, "同一トランザクションにしている2種類", accent=GREEN, items=[
...
note(s, ML + 6.33, 4.62, 6.0, 1.2, "テストで固定していること",
     ...
     accent=ACCENT, fill=ACCENT_L, size=10.5)
note(s, ML, 5.9, CW, 1.0, "Outbox にする理由",
     "スプレッドシートへの送信をその場で行うと、送信が失敗したときに日報の保存まで巻き戻すか、"
     "送信を諦めるかの二択になります。いったん送信予定としてデータベースに書いておけば、"
     "保存は完了させたうえで、送信は後から再試行できます。",
     accent=ACCENT, fill=ACCENT_L)

# ══════════════════════════════════════════════════════════════
# 16. スキーマ共通ルール
# ══════════════════════════════════════════════════════════════
s = sl_("44テーブルで守っている共通ルール", "表ごとに判断がぶれないよう、形を決めてある",
...
note(s, ML + 6.33, 5.6, 6.0, 1.3, "規約の副作用(第3章)",
...
sec_("第 3 章", "設計上の問題点(自己申告)",
     "気づいている弱点を6件挙げます。それぞれについてご意見をいただきたいです")
...
text(s, ML, 1.2, CW, 0.3,
     "外部予約システム(RESERVA)の顧客CSVを1項目も落とさず取り込む方針にしたため、CSVの列がほぼそのまま列になっている。",
     size=12, color=INK)
cats = [
...
for nm, body, col in future:
    ...
    yy2 += 0.7
card(s, ML + 6.33, yy2 + 0.06, 6.0, 1.35, "分けた場合の負担", accent=AMBER, items=[
...
note(s, ML, 6.13, CW, 0.8, "ご相談したいこと(相談①)",
     "39列は多すぎるので分けるべきか、顧客マスタなら39列は普通で分けるほうが害か。判断の基準"
     "(何列を超えたら、どういう単位で分けるか)をご教示ください。運用前なので、分けるなら今が最も安く直せます。",
     accent=RED, fill=RED_L, size=11)

# ══════════════════════════════════════════════════════════════
# 19. 問題② 平文化の代償
# ══════════════════════════════════════════════════════════════
s = sl_("問題② 業務データを平文にした代償", "検索性とAI活用を取り、保存時の保護を本番基盤に委ねた",
...
card(s, ML + 6.33, 1.25, 6.0, 1.85, "引き受けたもの", accent=RED, items=[
    ...
    {"t": "保存時の暗号化は本番の基盤(Cloud SQL)に依存。その本番がまだ無い"},
], body_size=10.5)
...
text(s, ML, 5.45, CW, 0.28, "現在動いている3つの環境", size=12, color=INK, bold=True)
...
text(s, ML, 1.2, CW, 0.3,
     "同じスタッフで時間帯が重なる予約は1件まで、という制約は、本来 PostgreSQL の除外制約(EXCLUDE)で"
     "守れる。それが使えていない。",
     size=12, color=INK)
...
text(s, ML, 2.96, 6.0, 0.5,
     "同じ法人・同じスタッフで時間の範囲が重なる行の禁止を、1行で表せる。"
     "アプリが同時に呼ばれても破れない。",
     size=10.5, color=MUTED, line=1.3)
...
bullets(s, ML, 3.82, 6.0, 1.3, [
    {"t": [("EXCLUDE には btree_gist 拡張が要る", {"bold": True}),
           ("。公開デモとテストで使っているブラウザ内PostgreSQL(PGlite)には、この拡張が存在しない"
            "(実機で確認済み)", {})]},
    {"t": "本番だけで有効にすると、テストが通っているのに本番だけ制約があるという、"
          "環境で形が違う状態になる"},
], size=10.5, line=1.3, gap=6)
...
opts = [
    ("A 本番だけ EXCLUDE を張る", "制約としては確実。ただしテスト環境と本番でスキーマが分岐する", AMBER),
...
note(s, ML, 5.18, 6.0, 0.9, "同種の制約が他にもある",
     "同じ顧客に同時刻の予約を2件入れない、なども範囲の重なりで表す種類の制約です。"
...
bullets(s, ML, 3.48, 6.0, 1.6, [
    ...
    {"t": "jsonb 1列のままなのは、1日分をまるごと読み書きする用途しかないため"},
], size=10.5, line=1.3, gap=6)
...
s = sl_("問題⑤ 小さな設計負債", "気づいているが、まだ手を付けていないもの",
        source="doc/db/overview.md 3.4節 / doc/db/new-domains.md 第7章 / README「実装状況」", accent=RED)
rows = [
...
note(s, ML, 6.0, CW, 0.9, "ご相談したいこと(相談⑤)",
     "この6件の優先順位付けが妥当かをご確認いただきたいです。ON DELETE と廃棄手順を最優先に置いていますが、"
     "運用開始前に整備すべきものと、動かしながら直せるものの線引きをご教示ください。",
     accent=RED, fill=RED_L)
...
sec_("第 4 章", "先行して用意したスキーマと、相談事項",
     "実装より先にDBの形だけ固めた5領域。そこで迷った判断と、相談事項のまとめ")
...
plans = [
    ("顧客カルテ", PINK, [["customer_notes", "customer_note_photos"],
                     ...
                     "customers の列にしないのは、上書きだといつ誰が入れた情報かが残らないため"]),
    ("予約(RESERVA移植)", ACCENT, [["service_menus", "reservations",
...
for ttl, col, items in plans:
    ...
    fill_text(rect(s, cx, 1.62, GW2, 0.42, fill=col, border=None, shape=MSO_SHAPE.RECTANGLE),
              ttl, size=10, color=WHITE, bold=True)
    names = items[0]
    nh = len(names) * 7 * 1.45 / 72  # 表名の行数ぶんだけ確保し、箇条書きはその下から
    text(s, cx + 0.11, 2.10, GW2 - 0.22, nh + 0.04, "\n".join(names), size=7, color=col,
         font=MONO, line=1.45)
    by = 2.10 + nh + 0.10
    bullets(s, cx + 0.11, by, GW2 - 0.22, 5.55 - by,
            [{"t": it, "s": 8.5} for it in items[1:]], size=8.5, line=1.3, gap=5,
            marker_color=col)
    cx += GW2 + GG2

card(s, ML, 5.78, 6.0, 1.12, "既存17枚と同じ規約に載せてある", accent=GREEN, items=[
...
# 26. 追加した5領域で迷った判断
# ══════════════════════════════════════════════════════════════
s = sl_("追加した5領域で迷った判断", "いずれも決め手がないまま選んで実装しています",
...
note(s, ML, 6.02, CW, 0.8, "ご相談したいこと(相談⑧)",
     ...
     "とくにEAVは項目が決まっていないことを理由に選んでおり、後戻りしにくい選択です。",
...
note(s, ML, 6.05, CW, 0.85, "指摘をいただきたい点",
     "⑨のように自分では気づきようがない落とし穴が、他にもあるかどうかを知りたいです。"
     "①〜⑧は選択肢が見えている判断ですが、⑨は見えていないものをご指摘いただく必要があります。",
     accent=ACCENT, fill=ACCENT_L)
...
card(s, ML, 1.28, 6.0, 2.6, "コード(こちらが正)", accent=ACCENT, items=[
    {"t": [("packages/db/src/schema/*.ts", {"font": MONO, "bold": True}),
           ("  44テーブルの定義。列ごとに設計の理由をコメントで書いてある", {})]},
...
table(s, ML, 4.35, CW, ["この資料での言い方", "正式な用語", "実装上の名前"],
      [["法人 / 会社", "テナント(tenant)", "tenants テーブル / tenant_id 列"],
       ...
       ["データベース側で行を絞り込む", "行レベルセキュリティ(RLS)", "pgPolicy('tenant_isolation') / FORCE ROW LEVEL SECURITY"],
