...
これまで `0000`〜`0015` の16本に分けて積んでいたマイグレーションを、`packages/db/drizzle/0000_baseline_schema.sql` の1本に統合した。理由は、本番環境がまだ配備されておらず(第1.3節)過去データを引き継ぐ必要が無いこと。稼働中のDBを段階的かつ安全に変更するための「1本ずつ積む」運用は、引き継ぐべきデータが存在して初めて意味を持つ。本番配備前の現時点では、最終形のスキーマを1本のDDLとして持つほうが、レビュー(本書もその一環)やローカル環境の再構築(`pnpm db:migrate`)を単純にできる。
