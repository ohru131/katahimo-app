...
既存の暗号化済みデータは引き継がない。当時のマイグレーション `0005_drop_field_encryption` で暗号化列を削除し、`0006_plaintext_columns` で平文列を追加し、`0007_receipts_dedupe_key_unique` で `(tenant_id, dedupe_key)` の部分一意インデックスを張った(`NOT NULL` 列には `DEFAULT ''`/`'{}'` を付けてあるので行が残っている DB でも適用は通るが、既存行の本文は空になる)。これらの番号付きマイグレーションは、2026-09のマイグレーション統合(第1.9節)で `packages/db/drizzle/0000_baseline_schema.sql` の1本にまとめられ、現在は個別のファイルとしては存在しない。
...
そこで「唯一の真実をDBに置く」方針に切り替え、`set_updated_at()` トリガー関数(当時の `0010_updated_at_trigger.sql`。現在は `0000_baseline_schema.sql` に統合済み。第1.9節参照)を `updated_at` 列を持つ全テーブルにBEFORE UPDATEトリガーとして張った。対象は当時 `customers`/`staff`/`daily_reports`/`accident_reports`/`family_members`/`attendance_days`/`app_settings`/`tenant_keys`/`outbox_jobs`/`coupons`/`coupon_redemptions` の11テーブル(`receipts`/`sessions`/`password_reset_codes` は `created_at` のみで更新経路が無いため対象外)。2026-09に追加した18テーブルのうち `updated_at` 列を持つものにも同じトリガーが張られており(`packages/db/drizzle/0000_baseline_schema.sql` 参照)、対象は現在29テーブルに増えている。アプリから `updatedAt` を明示指定しても、トリガーが常に `now()` で上書きする。
...
当時の `0008_add_missing_indexes.sql`(現在は `0000_baseline_schema.sql` に統合済み。第1.9節参照)で、`daily_reports`/`accident_reports`/`receipts` に `(tenant_id, customer_id, occurred_at DESC)`(`receipts` は `receipt_timestamp DESC`)、`family_members` に `(tenant_id, customer_id)`、`sessions` に `(tenant_id, staff_id)` の複合インデックスを追加した。`occurred_at`/`receipt_timestamp` をDESCで索引に含めるのは、`ORDER BY ... DESC LIMIT n` を並べ替えなしで返せるようにするため。リポジトリ層は実PostgreSQLに対する自動テストを持たないため、効果の検証は `EXPLAIN` で `Seq Scan` が `Index Scan` に変わることの手動確認にとどめている。

## 1.9 マイグレーションを1本に統合したこと(2026-09、本番未配備)

これまで `0001`〜`0010` の10本に分けて積んでいたマイグレーションを、`packages/db/drizzle/0000_baseline_schema.sql` の1本に統合した。理由は、本番環境がまだ配備されておらず(第1.3節)過去データを引き継ぐ必要が無いこと。稼働中のDBを段階的かつ安全に変更するための「1本ずつ積む」運用は、引き継ぐべきデータが存在して初めて意味を持つ。本番配備前の現時点では、最終形のスキーマを1本のDDLとして持つほうが、レビュー(本書もその一環)やローカル環境の再構築(`pnpm db:migrate`)を単純にできる。

このタイミングで、`packages/db/src/schema/customerNotes.ts`(顧客カルテ)・`reservations.ts`(予約)・`billing.ts`(請求・決済)・`optimization.ts`(訪問割当の最適化)・`transport.ts`(移動手当)の5ファイルに18テーブルを追加し、既存の `staff`/`daily_reports`/`receipts`/`coupon_redemptions` にも列・制約を追加した(詳細は第2章・第4章)。

本番配備後は、この「1本への統合」を再び行わない。以後のスキーマ変更は `pnpm db:generate` で `0001` 以降の差分マイグレーションを積み増す通常の運用に戻す(本番にデータが乗った後は、第1.3節と同じ理由で「引き継ぐデータが無い」という前提が使えなくなるため)。

## 1.10 区分値(CHECK制約の許可値)は `packages/shared/src/contracts/` に置く方針

第1.6節の方針(値域はDBのCHECK制約で縛る)を、追加した18テーブルにも一貫して適用した。許可値の配列そのものは `packages/shared/src/contracts/`(今回追加した `transport.ts`/`customerNotes.ts`/`reservations.ts`/`billing.ts`/`optimization.ts`。既存の `coupons.ts` 等も同じ形)にzodの `enum` として定義し、DB側は `packages/db/src/schema/_sqlLiteral.ts` の `sqlInList()` でその配列から `CHECK (col IN (...))` のDDLを組み立てる(第1.6節で述べた `outbox_jobs.kind` の手法を全面的に適用したもの)。
...
`customers`は実装上37列あるため、ER図には代表的な列のみを示す。全列の一覧は第4章の表を参照。2026-09に追加した18テーブルも同様に、PK・FK・区分値(CHECK制約のある列)・金額・一意制約に関わる列など代表的な列のみを示す(全列は各schemaファイルを参照)。1つの図に収めると読めない大きさになるため、業務ドメインごとに図を分割する(2.1〜2.5)。
...
erDiagram
    ...
    STAFF {
        ...
        text home_address "訪問割当の最適化用(2026-09追加)。自宅住所、ジオコーディングの入力(第2.5節)"
    ...
    DAILY_REPORTS {
        ...
        uuid customer_id FK "複合FK(tenant_id, customer_id)"
        uuid reservation_id "複合FK(tenant_id, reservation_id) → reservations(任意。2026-09追加、第2.3節)。UK(tenant_id, reservation_id) WHERE NOT NULLで1予約1日報"
...
erDiagram
    ...
    INVOICE_LINES {
        ...
        uuid receipt_id "複合FK・任意。UK(tenant_id, receipt_id) WHERE NOT NULL"
        uuid coupon_redemption_id "複合FK・任意。UK(tenant_id, coupon_redemption_id) WHERE NOT NULL"
    }
    ...
    PAYMENTS {
        ...
        string status "requires_payment_method/requires_action/processing/succeeded/canceled/failed。CHECK制約"
...
erDiagram
    ...
    CUSTOMER_TRAITS {
        ...
        uuid definition_id FK "複合FK(tenant_id, definition_id)"
...
| `staff` | スタッフ(認証情報を兼ねる) | ○ | `(tenant_id, id)`にUNIQUE。ログイン試行の絞り込み(`failed_login_attempts`/`locked_until`)もここ。訪問割当の最適化用に`home_address`/`home_lat`/`home_lng`/`preferred_transport_mode`を追加(2026-09追加。第2.5節) |
...
| `daily_reports` | 保育日報 | ○ | `staff`・`customers`双方への複合FK。本文は項目ごとの平文`text`列。開始・終了は`started_at`/`ended_at`(timestamptz)(2026-09 `doc/14` F項)。`reservation_id`で対応する予約に紐付け(任意。2026-09追加、第2.3節) |
| `accident_reports` | 事故報告/ヒヤリハット | ○ | 同上。本文は項目ごとの平文`text`列。対象児の生年月日は`target_dob_date`+`target_dob_raw`の2列(2026-09 `doc/14` F項) |
| `receipts` | 領収書登録 | ○ | `customer_id`はnullable(経費のみの領収書を許容)。重複検出は平文`dedupe_key`の等値一致。金額は`amount_yen`(整数)+`amount_raw`(生文字列)の2列、`billing_type`で顧客請求/会社経費を区別(2026-09 `doc/14` A項・第4章)。`(tenant_id, id)`にUNIQUE(`invoice_lines`からの複合FKの参照先。2026-09追加、第2.4節) |
...
| `coupon_redemptions` | 日報1件への割引クーポン適用記録 | ○ | `daily_reports`・`coupons`双方への複合FK。`(tenant_id, daily_report_id, coupon_id)`にUNIQUE(二重適用防止)。適用時点の割引条件をスナップショットして保持(2026-09 `doc/14` 4.1章)。`(tenant_id, id)`にUNIQUE(`invoice_lines`からの複合FKの参照先。2026-09追加、第2.4節) |
...
| `service_menus` | 予約時に選ぶサービスメニュー(所要時間・基本料金) | ○ | `(tenant_id, code)`にUNIQUE。値上げは行を書き換えず、過去の請求は`invoice_lines`側にスナップショット。第2.3節 |
| `reservations` | 予約(エンドユーザ向け予約システムの中心) | ○ | `status`/`source`はCHECK制約。状態とタイムスタンプの整合もCHECKで担保。`daily_reports.reservation_id`と1件まで対応。第2.3節 |
...
| `invoice_lines` | 請求明細(日報・領収書・クーポン適用のどれが根拠かを持つ) | ○ | `receipt_id`/`coupon_redemption_id`は部分UNIQUEで二重請求を防止。第2.4節 |
...
| `customer_traits` | 顧客側の特性の値 | ○ | `value_bool`/`value_int`/`value_text`はちょうど1つだけ非null(CHECK)。第2.5節 |
