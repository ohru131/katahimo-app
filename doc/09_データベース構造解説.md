...
`customers`は実装上39列あるため、ER図には代表的な列のみを示す。全列の一覧は第4章の表を参照。他のテーブルも同様に、PK・FK・区分値(CHECK制約のある列)・金額・一意制約に関わる列など代表的な列のみを示す(全列は各schemaファイルを参照)。1つの図に収めると読めない大きさになるため、業務ドメインごとに図を分割する(2.1〜2.5)。
...
erDiagram
    ...
    RECEIPTS {
        ...
        string dedupe_key "重複登録検出用。正規化済み文字列の等値一致。部分UNIQUE(tenant_id, dedupe_key) WHERE NOT NULL AND cancelled_at IS NULL"
        ...
        timestamp cancelled_at "取り消した時刻。nullなら有効。物理削除はしない(doc/14 §10)"
        text cancellation_reason "取消理由(任意の1行)"
        uuid cancelled_by_staff_id "複合FK・nullable。取り消した人。CHECK制約で3列の整合を縛る"
        timestamp mirror_claimed_at "ミラー送信を開始した時刻。取り消しと送信開始を同じ行のUPDATEで直列化する(doc/14 §10)"
    }
    ...
    COUPONS {
        ...
        string audience "all | assigned(customer_couponsで配った顧客だけ)。CHECK制約"
        string eligibility_kind "manual | birthday_month(対象者の誕生月のみ)。CHECK制約"
        string birthday_subject "customer | family_member | any。birthday_monthのときだけ必須(CHECK制約)"
        string usage_limit_kind "unlimited | once_per_customer | once_per_customer_per_year。CHECK制約"
    ...
    CUSTOMER_COUPONS {
        ...
        uuid tenant_id FK
        uuid customer_id FK "複合FK(tenant_id, customer_id)"
        uuid coupon_id FK "複合FK(tenant_id, coupon_id)。UK(tenant_id, customer_id, coupon_id)"
        date valid_from "この顧客に限った期間の下限。nullはマスタの期間に従う"
        date valid_to "この顧客に限った期間の上限。nullはマスタの期間に従う"
    }

    COUPON_REDEMPTIONS {
        uuid id PK
        uuid tenant_id FK
        uuid daily_report_id FK "複合FK(tenant_id, daily_report_id, customer_id)"
        uuid customer_id FK "同上。日報の顧客と違う値は入らない(使用上限の一意索引に使う)"
        ...
        string usage_limit_kind "同上"
        string usage_scope_key "上限を数える単位(lifetime/2026)。UK(tenant_id, coupon_id, customer_id, usage_scope_key) WHERE NOT NULL"
        string birthday_subject_name "誕生月クーポンの根拠にした人の氏名(適用時点の値)"
        date birthday_subject_dob "同上の生年月日"
    }
...
| `app_settings` | テナント単位の管理者設定(APIキー等) | ○ | 1テナント1行、`tenant_id`がPK。資格情報3列がリポジトリ内で唯一のアプリ層暗号化列(第3章)。領収書の締め日(`receipt_closing_day`/`receipt_mirror_lead_days`/`receipt_cancellable_days`)もここに置く。取り消し期限とミラー送信の開始時刻がこの3列から導かれる(`doc/14` §10) |
...
| 生年月日 | `dob_date`, `dob_raw` | 誕生月クーポン(`doc/14` §9)の判定に使う。RESERVA CSVに列が無いため取り込みでは埋まらず、顧客カルテからの手入力で入る(再取り込みでは上書きしない)。日付型と元表記の2列に分けるのは`doc/14` §6 |
