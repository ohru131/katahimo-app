...
export function createContainer(env: Env, db: Database): Container {
  ...
  return {
    ...
    appSettings,
    promptTemplates: new DrizzlePromptTemplateRepository(db),
    reportAiConfig: new DrizzleReportAiConfigRepository(db),
    customerReportProfiles: new DrizzleCustomerReportProfileRepository(db),
    ...
    // (doc/proposal/gas-bridge.md「新規GCP APIより既存GASブリッジを優先」)。未設定ならログに出すだけ。
