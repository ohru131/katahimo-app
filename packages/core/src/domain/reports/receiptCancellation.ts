...
export interface ReceiptDeadlinePolicy {
  ...
  /**
   * 締め日の何日前までにミラー送信を終えるか。
   *
   * 0にすると締め日当日の0時ぴったりに送ることになり、締め処理と競り合う。既定の1は
   * 「締め日の前日いっぱいで送信を終える」= 締める時点では全部シートに出ている状態。
   */
...
/**
 * その領収書のミラー送信を終えていなければならない最終日('YYYY-MM-DD'・JST)。
 * 締め日の `mirrorLeadDays` 日前。
 */
 ...
 *
 * 「領収書の日付 + cancellableDays」と「送信締切」の早いほう。締めるまでにシートへ出ている
 * ことを優先するので、締め間際の領収書はそのぶん取り消せる期間が短くなる。
 *
...
export function receiptCancellableUntil(receiptDateStr: string, policy: ReceiptDeadlinePolicy): string {
  ...
  const flushBy = receiptMirrorFlushBy(receiptDateStr, policy);
  return byDays < flushBy ? byDays : flushBy;
}
