 ...
 * occurredAt・stressLevel・esRating・reportType以外(=自由記述)をここにまとめる。
 * DB上は daily_reports/accident_reports テーブルの項目ごとの平文 text 列として保存する
 * (各フィールドが列と1:1に対応。SQLでの検索・集計・全文検索をそのまま行うため)。
 */
/**
 * doc/db/guidelines.md §6: 開始/終了時刻はDailyReportRecord/NewDailyReportInputの側(startedAt/endedAt。
 * stressLevel/esRatingと同じ並び)に持つ。文字列'HH:mm'のまま項目別列にしていた頃はここに
...
export interface AccidentReportContent {
  ...
  targetName: string;
  /** 対象児の生年月日(parseDateOnlyで解析できた場合のみ。'YYYY-MM-DD')。doc/db/guidelines.md §6。 */
