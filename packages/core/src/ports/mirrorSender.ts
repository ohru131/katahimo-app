...
/**
 * 【doc/db/guidelines.md §9】適用済みクーポン(coupon_redemptions)は含めない。ReceiptMirrorPayloadの
...
/**
 * 【doc/db/guidelines.md §10】billingType(請求区分)は含めない。gas-childcare-visit-appは稼働中の別システムで、
...
export interface ReceiptMirrorPayload {
  ...
  /** amountYenを整形した文字列。数値化できなければamountRaw、それも無ければ空文字(doc/db/guidelines.md §1)。 */
  amount: string;
  storeName: string;
  handoffText: string;
