// ==========================================
// 月次勤怠集計の計算ロジック(webapp-poc/server/attendanceCalc.js から移植)。
// 出勤簿テンプレート(出勤簿テンプレート.xlsx)の数式を再現した純粋計算関数群。
// PastSchedule.js の PAST_SCHEDULE_INPUT_COLUMNS で読み取った1日分の入力列(rowData)から、
// テンプレートの数式列(移動時間・待機時間・労働時間・残業時間・距離集計・超過回数)を
// 都度計算する。スプレッドシート/DBには一切依存しない(webapp-poc・本アプリ共通で使い回せる)。
//
// テンプレートの列対応(PAST_SCHEDULE_INPUT_COLUMNS参照):
 ...
 */
function computeMoveChain(prevEndTime, plannedMoveMin, weather, nextStartTime) {
    ...
    const weatherAdjustedMoveMin = planned === null ? null : weather === '雪' ? planned * SNOW_MOVE_MULTIPLIER : planned;
    ...
    return {
        ...
        weatherAdjustedMoveMin: weatherAdjustedMoveMin === null ? '' : Math.round(weatherAdjustedMoveMin * 100) / 100,
 ...
 */
function splitCoreAndOvertimeMinutes(startTime, endTime, isMtgException) {
 ...
 */
function computeLaborAndOvertime(rowData) {
    const blocks = [
 ...
 */
function countOverThreshold(distances) {
 ...
 */
function computeVisitCount(rowData) {
 ...
 */
function computeDistanceAggregates(rowData, leg1WeatherAdjustedMoveMin, leg2WeatherAdjustedMoveMin) {
    const j = toNumberOrNull(leg1WeatherAdjustedMoveMin) || 0;
    const s = toNumberOrNull(leg2WeatherAdjustedMoveMin) || 0;
 ...
 */
function computeDayDerived(rowData) {
    const data = rowData || {};
    ...
    const distanceAgg = computeDistanceAggregates(data, leg1.weatherAdjustedMoveMin, leg2.weatherAdjustedMoveMin);
