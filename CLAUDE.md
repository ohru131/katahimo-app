...
- **資料には現時点の仕様だけを書く。**「今回こう直した」「前はこうだった」という
  ...
  まだ社外に出していないため)
  - 例外は `doc/proposal/` のように**日付を持つ記録**。提案書はその時点で何を提案したかの
    記録なので、後から現状に合わせて書き換えない。そこから変えた判断
    (認証を Firebase Auth ではなく自前実装にした等)は、提案を読む人に必要なので
    `doc/slides/architecture.pptx` 側に「提案書から変更した点」として明示する

## PR

- マージは **Squash and merge**
- CodeRabbit のレビューを受け、指摘に対応してからマージする
- **CodeRabbit がレビューしている間は push しない。** 途中でheadが変わるとレビューが
  止まってしまう。レビューが出揃ってから、指摘への対応とその間に溜めた変更を
  まとめて1回で push する
